...
namespace VisionCraft::Engine
{
    ...
    void ThresholdNode::Process()
    {
        ...
        try
        {
            ...
            LOG_INFO("ThresholdNode {}: Applied thresholding (threshold: {}, actual: {}, max: {}, type: {})",
                     GetName(), threshold, actualThreshold, maxValue, typeStr);
        }
    ...
    {
        if (typeStr == "THRESH_BINARY") return cv::THRESH_BINARY;
        if (typeStr == "THRESH_BINARY_INV") return cv::THRESH_BINARY_INV;
        if (typeStr == "THRESH_TRUNC") return cv::THRESH_TRUNC;
        if (typeStr == "THRESH_TOZERO") return cv::THRESH_TOZERO;
        if (typeStr == "THRESH_TOZERO_INV") return cv::THRESH_TOZERO_INV;
        if (typeStr == "THRESH_OTSU") return cv::THRESH_OTSU;
        if (typeStr == "THRESH_TRIANGLE") return cv::THRESH_TRIANGLE;
